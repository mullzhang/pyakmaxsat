...
class CNF_Formula {
    ...
    TL *W_large;
    //! sum of clause weights of clauses containing literal i contained in
    //! inconsistent subformulas detected in lower bound calculation
    TL *W_lb;
    ...
    int *literal_data;
    //! stores the clause id of clauses which contain the variables (a, b) at
    //! position a * nVars + b
    int *ternary_clause_data;
    //! stores the literals in the order in which they should be selected for
    //! propagation
    pair<TL, int> *literal_order;
    //! length of literal_order list
    int l;
    //! stores a stack of processed variables during lower bound calculation
    ...
    int maxVn;
    //! maps a variable in compacted variable numbering to the original
    //! variables
    int *mapping;
    ...
    inline bool normalize_clause_array(vector<int_c> &clause) const {
        ...
        for (int i = 0; i < (int)clause.size(); ++i) {
            ...
            // check if the clause is a tautology (it contains clause[i] and
            // -clause[i])
    ...
    inline int propagateLiteral(int L) {
        ...
        for (vector<int>::reverse_iterator it =
             ...
             it < appears2.rend(); ++it) {
            if (all_clauses.getDeleteFlag(*it)) {
                // in this case, it may be a clause to be deleted, or one which
                // was used in an inconsistent subformula to increase the lower
                // bound we can't distinguish this, so we just move the clause
                // pointer to the end of the list note that we do not need to
                // consider such clauses again when looking for inconsistent
                // subformulas until lower bound calculation has ended and the
                // delete flag gets removed
                --appears_len[L];
        ...
        if (literal_data[L] > 0) {
            assert(literal_data[L] != UNIT_CLAUSE &&
                   !all_clauses.getDeleteFlag(literal_data[L]));
            return literal_data[L];
        }
        // if there is a unit literal L, it conflicts with propagating -L
        if (W_unit[L] > 0) return literal_data[L] = UNIT_CLAUSE;
        return 0;
    }
    ...
    inline void addLiteral(int L) {
        ...
        for (vector<int>::reverse_iterator it = appears2.rbegin();
            ...
            if (all_clauses.getLength(*it) == 1) {
                ...
                // time stamp appears_traversed[L]
            ...
            // increase the length of the clause (literal L will be added back
            // to the clause)
    ...
    inline int addLiteralConflict(int L) {
        ...
        all_clauses.unassignVariable(L);
        if (!propagated) return W_unit[L] > 0;
        return propagated;
    }

    //! apply resolution to pairs of binary clause (x_i, x_j) (x_i, -x_j) or
    //! ternary clauses (x_i, x_j, x_k) (x_i, x_j, -x_k)
    inline TL binary_ternary_resolution(int L) {
        ...
        // store in ternary_clause_data[(L1+nVars)*(2*nVars+1)+L2+nVars] the
        // clause consisting of literals L, L1 and L2
        for (int i = appears[L].size() - 1; i >= 0; --i) {
            ...
            if (all_clauses.getDeleteFlag(it)) {
                ...
                if (all_clauses.getLength(it) == 2) {
                ...
                } else if (ternary_clause_data != NULL &&
                           all_clauses.getLength(it) == 3) {
                    literals = all_clauses.getLiterals(it);
                    other1 = other2 = 0;
                    // determine the other two literals other1 and other2 which
                    // appear together with literal L in the clause
                    for (int i = 0; i < 3; ++i)
                        if (literals[i] != L) {
                            if (!other1)
                                other1 = literals[i];
                    ...
                    if (other1 > other2) swap(other1, other2);
                    // calculate a unique id for a clause containing literal L
                    // and literal other1 and literal other2
                    ...
                    if (ternary_clause_data[id] > 0) {
                        ...
                        ULL w = all_clauses.getWeight(it);
                        assert(all_clauses.getWeight(ternary_clause_data[id]) >
                               0);
        ...
        // now do the resolution using literal_data and ternary_clause_data
        // arrays
        for (int i = appears[L].size() - 1; i >= 0; --i) {
            int &it = appears[L][i];
            assert(!all_clauses.getSpecialFlag(it));
            if (all_clauses.getDeleteFlag(it)) {
                // in this case it can only be a clause which was deleted
                // because it is the same as another clause, and its weight was
                // added to the other clause we only need to delete the clause
                // pointer
                it = appears[L].back();
                appears[L].pop_back();
            } else {
                if (all_clauses.getLength(it) == 2) {
                    ...
                    if (literal_data[other1] != it) {
                        assert(all_clauses.getWeight(it) ==
                               all_clauses.getSavedWeight(it));
                        continue;
                    }
                    ...
                    if (all_clauses.getSavedWeight(it) != w) {
                        ...
                        rlist.addEntry(it);
                        rlist.commit(timestamp++,
                                     w - all_clauses.getSavedWeight(it), true);
                        all_clauses.saveWeight(it);
                    }
                    // check if there is also a clause consisting of L and
                    // -other1 -> resolution of (L, other1) with (L, -other1)
                    if (literal_data[-other1] > 0) {
                        ...
                        if (all_clauses.getSavedWeight(c) != w2) {
                            ...
                            rlist.addEntry(c);
                            rlist.commit(timestamp++,
                                         w2 - all_clauses.getSavedWeight(c),
                                         true);
                        ...
                        cnt += wmin;
                        if (w == wmin) {
                            // if the complete weight of the clause was
                            // subtracted, the clause pointer can be deleted
                            it = appears[L].back();
                            appears[L].pop_back();
                            continue;
                        }
                    ...
                    if (other1 > other2) swap(other1, other2);
                    id = (other1 + nVars) * (nVars * 2 + 1) + other2 + nVars;
                    if (ternary_clause_data[id] <= 0) continue;
                    assert(ternary_clause_data[id] == it);
                    ...
                    if (other3 > other4) swap(other3, other4);
                    ...
                    if (ternary_clause_data[id] > 0) {
                        ...
                        ULL wmin = min(w, w2);
                        assert(wmin > 0);
                        if (wmin == w2) ternary_clause_data[id] = 0;
                        all_clauses.subtractWeight(c, wmin, true);
                        all_clauses.subtractWeight(it, wmin, true);
                        rlist.addEntry(c);
                        rlist.addEntry(it);
                        ...
                        if (literal_data[other2] > 0) {
                            assert(all_clauses.getWeight(literal_data[other2]) >
                                   0);
                        ...
                        // if clause weight has become zero, the clause pointer
                        // can be deleted
                        if (w == wmin) {
                            it = appears[L].back();
                            appears[L].pop_back();
                    ...
                    if (ternary_clause_data[id] > 0) {
                        ...
                        rlist.commit(timestamp++, wmin, false);
                        if (literal_data[other1] > 0) {
                            assert(all_clauses.getWeight(literal_data[other1]) >
                                   0);
                        ...
                        // if weight of the clause has become zero, the clause
                        // pointer can be deleted
                        if (w == wmin) {
                            it = appears[L].back();
                            appears[L].pop_back();
                            continue;
        ...
        // take_back contains the binary clauses created by resolution of
        // ternary clauses
        if (!take_back.empty()) {
            appears[L].insert(appears[L].end(), take_back.begin(),
                              take_back.end());
            take_back.clear();
        }
        appears_len[L] = (int)appears[L].size();
    ...
    void resolveConflict() {
        ...
        assert(which.empty());
        // pos2 is either -1 or indicates that we may shorten the inconsistent
        // subformula by taking only the first "pos2" entries and adding a unit
        // literal "var"
        ...
        for (int *it = vars + vars_top; it >= vars; --it) {
            ...
            if ((t = addLiteralConflict(-*it))) {
                // add literal to the list of literals which need to be
                // propagated to yield the conflict clause
                which.push_back(*it);
                if (t > maxt) maxt = t;
                if (t == 2) {
                    // this means that the literal -*it occured in two marked
                    // clauses this may be the "starting point" of a cycle
                    // structure save the position
                    if (take_back.size() == 3) pos = 3;
                    // save the literal
                    other = *it;
                }
                // if -*it occurs in more than one marked clause, we may not
                // shorten the inconsistent subformula after this point
                if (t > 1) pos2 = -1;
                if (literal_data[*it] != UNIT_CLAUSE) {
                    // check if there exists a unit literal which could be used
                    // for propagation instead of the current clause
                    ...
                    // mark the clause and add it to the list of clauses of the
                    // inconsistent subformula
                    all_clauses.addMarker2Clause(literal_data[*it]);
                    take_back.push_back(literal_data[*it]);
                } else {
                    if (W_unit[*it] < (TL)minweight) minweight = W_unit[*it];
                    // represent a unit literal by *it - nVars
                    // it holds -nVars <= *it <= nVars, thus *it - nVars <= 0 <
                    // UNIT_CLAUSE
                    take_back.push_back(*it - nVars);
                    // if we encounter another used unit literal, we cannot
                    // shorten the inconsistent subformula beyond that
                    if (pos2 >= 0) {
                        ++pcnt;
                        if (pcnt > 1) pos2 = -1;
                    }
        ...
        if (pos2 >= 2 && pos2 + 1 < (int)take_back.size() &&
            W_unit[var + nVars] >= (TL)minweight) {
            int cnt = -1;
        ...
        int cnt = 0;
        minweight = MAXWEIGHT;
        // determine the minimum weight of a clause belonging to the
        // inconsistent subformula also determine the position of the first
        // clause with more than two literals
        for (vector<int>::const_iterator it = take_back.begin();
            ...
            if (*it > UNIT_CLAUSE) {
                ...
                if (first_large_clause < 0 && all_clauses.getLength(*it) > 2)
                    first_large_clause = distance(
                        (vector<int>::const_iterator)take_back.begin(), it);
            } else {
                if (W_unit[*it + nVars] < (TL)minweight)
                    minweight = (ULL)W_unit[*it + nVars];
                ++cnt;
            }
        // remove the markers from the clauses and subtract the weight from unit
        // clauses
        for (vector<int>::const_iterator it = take_back.begin();
             it != take_back.end(); ++it) {
            if (*it <= 0) {
        ...
        if (first_large_clause >= 0) {
            ...
            if (first_large_clause > 2 && take_back.size() > 3) {
                ...
                if (c[0] > UNIT_CLAUSE && all_clauses.getLength(c[0]) == 2 &&
                    c[1] > UNIT_CLAUSE && all_clauses.getLength(c[1]) == 2 &&
                    c[2] > UNIT_CLAUSE && all_clauses.getLength(c[2]) == 2) {
                    int other = which[2];
                    ...
                    if (literals2[0] == literals3[0])
                        ...
                        i1 = i2 = 1;
                    if (i1 >= 0 && ((literals1[0] == -literals2[1 - i1] &&
                                     literals1[1] == -literals3[1 - i2]) ||
                                    (literals1[0] == -literals3[1 - i2] &&
                                     literals1[1] == -literals2[1 - i1]))) {
                        assert(literals1[0] != -other &&
                               literals1[1] != -other);
                        assert(literals2[i1] == -other);
                        other = literals2[i1];
                        assert(pos == 3);
                        // do cycle resolution
                        if (process_cycle_clauses(c[0], c[1], c[2],
                                                  literals2[i1], literals1[0],
                                                  literals1[1])) {
                            // in the special case of unit propagation, we do
                            // not need to include the newly generated ternary
                            // clauses in the inconsistent subformula
                            take_back.pop_back();
                            take_back.pop_back();
                            // remove the binary clauses which were used in the
                            // resolution
                            take_back.erase(take_back.begin(),
                                            take_back.begin() + 3);
            ...
            for (vector<int>::const_iterator it = take_back.begin();
                ...
                if (*it > 0) {
                    ...
                    if (all_clauses.getLength(*it) == 2) {
                        ...
                        const int *literals = all_clauses.getLiterals(*it);
                        for (int j = all_clauses.getLength(*it) - 1; j >= 0;
                             --j)
        ...
        if (cnt == 2) {
            ...
            if (added) rlist.commit(timestamp++, minweight, true);
            ...
            assert(literals[0] != other && literals[1] != other &&
                   literals[0] != -other && literals[1] != -other);
            ULL mw = min(all_clauses.getWeight(take_back[0]),
                         min(all_clauses.getWeight(take_back[1]),
                             all_clauses.getWeight(take_back[2])));
            assert(mw >= minweight);
            if (mw > minweight) {
            ...
            do_insert(nC[2], c);
            rlist.addEntry(c);
            if (mw > minweight) rlist.commit(timestamp++, mw, true);
            bool added = false;
            for (vector<int>::const_iterator it = take_back.begin() + 3;
                 it + 1 < take_back.end(); ++it) {
                const int *literals = all_clauses.getLiterals(*it);
                nC[0] = -literals[0];
                nC[1] = -literals[1];
    ...
    void restoreClauses(int L) {
        ...
        while ((rclauses = rlist.retrieve(appears_traversed[L], length,
                                          deletion_time, minweight, sign)) !=
               NULL) {
            while (length > 0) {
                --length;
                int clause_id = rclauses[length];
                if (clause_id <= 0) {
                    // in that case, we have a unit clause with a unit literal
                    // clause_id + nVars
                    clause_id += nVars;
                ...
                if (all_clauses.getDeleteFlag(clause_id)) {
                    ...
                    for (int i = all_clauses.getLength(clause_id) - 1; i >= 0;
                        ...
                        for (vector<int>::const_iterator it =
                             ...
                             it != appears[literals[i]].end(); ++it)
                            if (*it == clause_id) ++cnt;
                        assert(cnt == 1);
                    }
                    for (int i = -nVars; i <= nVars; ++i) {
                        bool valid = (i != 0);
                        for (int j = all_clauses.getLength(clause_id) - 1;
                             j >= 0; --j)
                            if (literals[j] == i) valid = false;
                        if (!valid) continue;
                        for (vector<int>::const_iterator it =
                                 appears[i].begin();
                             it != appears[i].end(); ++it)
                    ...
                    assert(all_clauses.getWeight(clause_id) ==
                               all_clauses.getSavedWeight(clause_id) &&
                           all_clauses.getSpecialFlag(clause_id) &&
                           all_clauses.getDeleteFlag(clause_id));
                    continue;
                }
    ...
    inline bool process_cycle_clauses(int c1, int c2, int c3, int l1, int l2,
                                      int l3) {
        int_c nC[3];
        if (l1 == l2 || l1 == l3 || l2 == -l3) return false;
        assert(l1 != l2 && l1 != -l2 && l1 != l3 && l1 != -l3 && l2 != l3 &&
               l2 != -l3);
        assert(all_clauses.getLength(c1) == 2 &&
               all_clauses.getLength(c2) == 2 &&
               all_clauses.getLength(c3) == 2);
        if (!all_clauses.getDeleteFlag(c1) && !all_clauses.getDeleteFlag(c2) &&
            !all_clauses.getDeleteFlag(c3)) {
            ULL minweight =
                min(all_clauses.getWeight(c1),
                    min(all_clauses.getWeight(c2), all_clauses.getWeight(c3)));
            assert(minweight > 0);
            // save the clause pointers to clauses involved in the cycle
            // resolution
    ...
    void do_sort(int i) {
        ...
        for (vector<pair<ULL, int> >::iterator it = temp.begin();
             it != temp.end(); ++it)
            *it2++ = it->second;
    }
    ...
    inline bool detectFailedLiteral(int L) {
        ...
        if (propagateLiteral(-L)) return true;
        ...
        while (head != tail) {
            ...
            if (propagateLiteral(-nexti)) {
                // clear the queue
                while (head != tail) {
                    if (Q[head] != -nexti) literal_data[Q[head]] = 0;
                    ++head;
                }
                return true;
            }
        ...
        while (1) {
            ...
            if (!found) {
                ...
                for (int ii = 0, tiv = iv; !found && ii < l; ++ii) {
                    ...
                    if (--tiv < 0) tiv = l - 1;
                    assert(i != 0);
                    if (W_binary[-i] > W_binary[i] ||
                        (W_binary[-i] == W_binary[i] &&
                         W_large[-i] > W_large[i]))
                        i = -i;
                    assert(!assigned_values[abs(i)]);
                    minii = ii;
                    if (literal_data[i] > 0 || literal_data[-i] > 0) continue;
                    ...
                    assert(head == tail);
                    head = tail = save_head;
                    for (int *it = vars + vars_top; it > vars + save_vars_top;
                         --it) {
            ...
            while (head != tail) {
                // for unit propagation, move a unit literal towards the front
                // which yields an immediate conflict first check that the
                // literal at the front of the queue does not yield an immediate
                // conflict
                if (!fl && W_unit[-Q[head]] == 0 &&
                ...
                while (head != oldtail) {
                    ...
                    if (propagateLiteral(-L)) {
                        ...
                        // reset the literal_data array to 0 at positions still
                        // in the queue
                        while (head != tail) {
                            if (Q[head] != -L) literal_data[Q[head]] = 0;
                            ++head;
                        }
                        iv -= (1 + minii);
                        if (iv < 0) iv += l;
    ...
    void resolveConflictFl() {
        ...
        assert(literal_data[-vars[vars_top]] > 0);
        assert(take_back.empty());
        // add the conflict clause to the inconsistent subformula to be
        // constructed
        take_back.push_back(literal_data[-vars[vars_top]]);
        literal_data[-vars[vars_top]] = 0;
        int t, pos = 0;
        ...
        for (int *it = vars + vars_top; it >= vars; --it) {
            ...
            if ((t = addLiteralConflict(-*it))) {
                ...
                // literal -*it this means that t > 0 implies the current clause
                // is needed to derive the conflict clause
                if (literal_data[*it] != UNIT_CLAUSE) {
                    assert(!all_clauses.getMarker(literal_data[*it]));
                    all_clauses.addMarker2Clause(literal_data[*it]);
                    take_back.push_back(literal_data[*it]);
                    // collect the first three clauses to check for cycle
                    // resolution
                    if (pos >= 0 && pos < 3) c[pos++] = take_back.back();
                } else if (W_unit[*it] > 0) {
                    if (pos < 3) pos = -1;
                    take_back.push_back(*it - nVars);
                } else {
                    ...
                    for (vector<int>::const_iterator it2 =
                             unit_implication_list[-*it].begin();
                         it2 != unit_implication_list[-*it].end(); ++it2) {
                        assert(*it2 > UNIT_CLAUSE &&
                               !all_clauses.getDeleteFlag(*it2));
        ...
        assert(minw > 0);
        // process unit implication lists belonging to the inconsistent
        // subformula and look for clauses forming a cycle
        for (vector<int>::const_reverse_iterator it = which.rbegin();
             it != which.rend(); ++it) {
            if (unit_implication_list[*it].size() >= 3) {
                // check if the three clauses form a cycle
                int c1 = unit_implication_list[*it][0];
                if (c1 <= UNIT_CLAUSE ||
                    find(cycle_clauses.begin(), cycle_clauses.end(), c1) !=
                        cycle_clauses.end()) {
                    unit_implication_list[*it].clear();
                    continue;
                }
                int c2 = unit_implication_list[*it][1];
                if (c2 <= UNIT_CLAUSE ||
                    find(cycle_clauses.begin(), cycle_clauses.end(), c2) !=
                        cycle_clauses.end()) {
                    unit_implication_list[*it].clear();
                    continue;
                }
                int c3 = unit_implication_list[*it][2];
                if (c3 <= UNIT_CLAUSE ||
                    find(cycle_clauses.begin(), cycle_clauses.end(), c3) !=
                        cycle_clauses.end()) {
                    unit_implication_list[*it].clear();
                    continue;
                }
        ...
        // check if we can use cycle resolution
        if (pos == 3 && all_clauses.getLength(c[0]) == 2 &&
            all_clauses.getLength(c[1]) == 2 &&
            all_clauses.getLength(c[2]) == 2 &&
            !binary_search(cycle_clauses.begin(), cycle_clauses.end(), c[0]) &&
            !binary_search(cycle_clauses.begin(), cycle_clauses.end(), c[1]) &&
            !binary_search(cycle_clauses.begin(), cycle_clauses.end(), c[2])) {
            const int_c *literals1 = all_clauses.getLiterals(c[0]);
            const int_c *literals2 = all_clauses.getLiterals(c[1]);
            const int_c *literals3 = all_clauses.getLiterals(c[2]);
            // first find common literals in literals2 and literals3
            ...
            if (i1 >= 0 && ((literals1[0] == -literals2[1 - i1] &&
                ...
                int other = literals2[i1];
                // do cycle resolution
                if (process_cycle_clauses(c[0], c[1], c[2], literals2[i1],
                                          literals1[0], literals1[1])) {
                    assert(W_unit[other] >= minw);
                    literal_data[other] = 0;
                    W_unit[other] -= minw;
        ...
        sort(cycle_clauses.begin(), cycle_clauses.end());
        which.clear();
        // subtract minw from all clauses belonging to the inconsistent
        // subformula
        for (vector<int>::const_iterator it = take_back.begin();
             it != take_back.end(); ++it)
            if (*it <= 0) {
            ...
            } else if (!binary_search(cycle_clauses.begin(),
                                      cycle_clauses.end(), *it)) {
                if (all_clauses.getWeight(*it) >= bestCost) continue;
        ...
        // now process all remaining unit implication lists and look for clauses
        // forming a cycle
        for (int *it = vars + vars_top; it >= vars; --it) {
            // check if we can use cycle resolution
            if (unit_implication_list[-*it].size() >= 3) {
    ...
    int resolveFailedLiteral(int save_vars_top) {
        ...
        for (vector<int>::const_iterator it = which.begin();
             it + 1 < which.end(); ++it) {
            assert(*it > UNIT_CLAUSE && all_clauses.getLength(*it) > 1 &&
                   all_clauses.getWeight(*it) > 0);
            all_clauses.removeMarkerFromClause(*it);
        }
        which.resize(pos);
    ...
    double clause_height(int start, int skip, int satisfied) {
        ...
        if (satisfied) return wa - potential;
        double difference = 1e34;
        for (int i = 0; i < len; i += 2) {
            if (start + i == skip) continue;
            double tdiff = fabs(psi[start + i] - psi[start + i + 1]);
            if (tdiff < difference) difference = tdiff;
        }
        if (wa > difference) return wa - potential - difference;
        return -potential;
    }
    ...
    void generalized_unit_propagation() {
        ...
        for (int i = 1; i <= nVars; ++i) {
            if (!assigned_values[i]) {
                ...
                literal_order[l].first =
                    (W_binary[i] + W_large[i]) * (W_binary[-i] + W_large[-i]);
                ++l;
            }
        }
        ...
        while (needed_for_skip > 0) {
            if (!detectConflictFl(fl, iv, iv2)) break;
            // check if failed literals were used; if not, transformation rules
   ...
   public:
    ...
    CNF_Formula(istream &istr) {
        ...
        while (getline(istr, line)) {
            ...
            if (line[0] == 'p' && line[1] == ' ') {
                t = sscanf(line.c_str(), "p %s %d %d %llu", type, &maxVn,
                           &nClauses, &hard);
                if (t >= 3) break;
            }
        }
        ...
        for (int i = 0; i < nClauses; ++i) {
            ...
            while (istr >> var && var) clause.push_back(var);
            // now remove duplicate literals, and check if the clause is a
            // tautology
            if (normalize_clause_array(clause)) {
                // if it is not a tautology, store the clause data
                for (vector<int_c>::iterator it = clause.begin();
                     it != clause.end(); ++it) {
                    int sign = 1;
                    int var = *it;
                    if (var < 0) {
                    ...
                    // first, all literals from all clauses are put into the
                    // literals vector
        ...
        if (nVars != maxVn)
            printf(
                "c Number of variables occuring in the formula: %d max "
                "variable = %d -> remapping\n",
                nVars, maxVn);
        ...
        ternary_clause_data =
            (nVars > 5000 ? NULL : new int[(2 * nVars + 1) * (2 * nVars + 1)]);
        ...
        if (ternary_clause_data != NULL)
            memset(ternary_clause_data, 0,
                   sizeof(int) * (2 * nVars + 1) * (2 * nVars + 1));
        ...
        // since we want to index with values in the range [-nVars, nVars], add
        // +nVars to the pointers
        W_unit += nVars;
        W_binary += nVars;
        W_large += nVars;
        W_unit_save += nVars;
     ...
     */
    inline TL getW_lb(int i) const { return W_lb[i]; }
    //! get number of variables
    inline int getNVars() const { return nVars; }
    ...
    inline vector<int> getSolution() const {
        ...
        set<int> used;
        for (int i = 1; i <= nVars; ++i) used.insert(mapping[i]);
        for (int i = 1; i <= maxVn; ++i)
            if (used.find(i) != used.end()) {
                ++j;
                assert(j <= nVars);
                solution[j - 1] = (int)bestA[j];
            }
    ...
    inline void printSolution() const {
        ...
        for (int i = 1; i <= maxVn; ++i)
            ...
            if (used.find(i) == used.end()) printf(" %d", i);
    ...
    inline TL getLength(int L) const {
        ...
        for (vector<int>::const_iterator it = appears[L].begin();
             it != appears[L].end(); ++it)
            if (all_clauses.getDeleteFlag(*it) == false &&
    ...
    void unassignLiteral() {
        ...
        for (vector<int>::const_iterator it = appears[L].begin();
            ...
            if (l == 2) {
                ...
                ULL w = all_clauses.getWeight(*it);
                for (int i = 0; i < l; ++i) W_large[literals[i]] += w;
            }
            all_clauses.removeDeleteFlagFromClause(*it);
            ...
            for (int i = 0; i < l; ++i) {
                ...
                for (vector<int>::const_iterator it2 =
                         appears[literals[i]].begin();
                     it2 != appears[literals[i]].end(); ++it2)
                    if (*it2 == *it) {
                        found = true;
                        break;
                    }
    ...
    /*! \returns true iff lower bound plus current cost exceeds the best
     * solution \remark cost may be increased resulting from formula
    ...
    ULL bestMinusLowerBound() {
        ...
        if (needed_for_skip > 0) generalized_unit_propagation();
        // go through all changed clauses and reset their weights which were
        // possibly changed by the generalized unit propagation
        ULL diff;
        for (vector<int>::const_iterator it = changed.begin();
             it != changed.end(); ++it) {
            if ((diff = all_clauses.getSavedWeight(*it) -
                        all_clauses.getWeight(*it)) == 0)
                continue;
            const int_c *literals = all_clauses.getLiterals(*it);
        ...
        for (int i = 1; i <= nVars; ++i) {
            ...
            if (appears_len[-i] < (int)appears[-i].size()) {
                reverse(appears[-i].begin() + appears_len[-i],
                        appears[-i].end());
                appears_len[-i] = appears[-i].size();
            }
    ...
    inline ULL getBestCost() const { return bestCost; }
